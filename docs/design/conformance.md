...
| Q18      | Select records for indexing inside a larger repository, then edit unrelated files, change scope, or delete the final record. | Preserve the full reference root; persist selections; currency ignores unselected files; explicit updates reconcile scope and confirmed deletions. Failed scope changes cannot claim current currency. |
| Q19      | Reuse a caller-owned store for repeated high-level search and an ordinary index update.                                      | Preserve native hybrid/lexical behavior and updated SQLite visibility; never close a borrowed store, including on failed queries. Rebuild requires explicit close/reopen.                              |
