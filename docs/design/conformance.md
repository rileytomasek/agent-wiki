...
| Q15      | Indexed hit with headings, citations, Unicode, and moved source.        | Use indexed body/context and original indexed path mapping, without per-result current-source reconciliation warnings.         |
| Q16      | Hit entirely in generated metadata.                                     | Do not invent a source-body position.                                                                                          |
...
| C06      | Every authored TypeScript area and every production source file.                  | Strict checking includes tests/config/scripts; coverage includes CLI and unimported production files at the required floors. |
