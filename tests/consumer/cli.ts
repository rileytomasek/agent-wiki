...
await mkdir(empty);
command(empty, ['index', 'records']);
const indexed = command(empty, ['status']);
assert.ok('status' in indexed);
assert.equal(indexed.status, 'current');
assert.ok('selections' in indexed);
assert.deepEqual(indexed.selections, ['records']);
command(empty, ['index', '--rebuild', '.']);
const whole = command(empty, ['status']);
assert.ok('selections' in whole);
assert.deepEqual(whole.selections, []);
